...
const Employee = ({ inModal = false, onSuccess = null, close }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setTimeout(() => {
        onSuccess?.();
        close?.(); // Close the modal automatically after success
      }, 1500);
  ...
  return (
    <div
      className={
        inModal
          ? "p-6"
          : "min-h-screen bg-gray-50 flex items-start justify-center py-12 px-4"
      }
    >
      <div
        className={
          inModal
            ? "w-full"
            : "w-full max-w-md bg-white rounded-2xl shadow-sm border border-gray-200 p-8"
        }
      >
        {/* Header */}
        <div className="mb-6">
